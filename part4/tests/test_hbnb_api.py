...
def app():
    """Crée une instance de l'app avec SQLite fichier pour chaque test."""
    # Supprime la DB de test si elle existe déjà
    ...
    with application.app_context():
        ...
        # Réinitialise le facade pour pointer sur la session de test
...
def admin_token(app, client):
    """Crée un admin via un appel API interne puis login."""
    # On passe par le client Flask — il partage le même contexte/session que l'app
    from app.models.user import User
    from app import db as _db

    # Insertion directe dans la session active du contexte de test
    with app.app_context():
        ...
        _db.session.commit()
        admin_id = admin.id  # on lit l'id avant de quitter le contexte

    # Vérification que le login fonctionne
...
class TestAuth:
    ...
    def test_login_success(self, client, admin_token):
        ...
        # admin_token a déjà loggé, on vérifie juste que le token est valide
...
class TestPlaces:
    ...
    def test_create_place_invalid_owner(self, client):
        ...
        # TODO: places.py ne vérifie pas l'existence de l'owner — à corriger
...
class TestReviews:
    ...
    def test_create_review_invalid_user(self, client):
        ...
        # TODO: reviews.py ne vérifie pas l'existence du user — à corriger
        assert resp.status_code in [201, 400, 404]
